document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (path.includes('products.html')) {
        ...
        const cart = JSON.parse(localStorage.getItem('cart')) || [];
        const newProducts = cart.filter(item => item.condition === 'New');
        const orderSummaryList = document.getElementById('order-summary-list');
        const subtotalAmountSpan = document.getElementById('subtotal-amount');
        const deliveryChargeAmountSpan = document.getElementById('delivery-charge-amount');
        const totalPayableAmountSpan = document.getElementById('total-payable-amount');
        ...
        if (orderSummaryList) {
            if (newProducts.length > 0) {
                ...
                newProducts.forEach(product => {
                    const imageUrl = product.image ? `http://localhost:3000/uploads/${product.image.split(',')[0].trim()}` : 'https://placehold.co/50x50';
                    subtotal += product.price * product.quantity;
                    productsHtml += `
                        <div class="d-flex align-items-center mb-2">
                            <img src="${imageUrl}" alt="${product.name}" style="width: 50px; height: 50px; object-fit: cover; margin-right: 10px; border-radius: 5px;">
                            <div>
                                <h6 class="mb-0">${product.name} (x${product.quantity})</h6>
                                <p class="text-muted mb-0">৳${(product.price * product.quantity).toFixed(2)}</p>
                            </div>
                        </div>
                    `;
                });
                orderSummaryList.innerHTML = productsHtml;
            } else {
                orderSummaryList.innerHTML = '<p>No new products in cart for checkout.</p>';
            }
        }

        // Function to update totals
        function updateTotals() {
            if (deliveryLocationSelect.value === 'inside_dhaka') {
                deliveryCharge = 80;
            } else if (deliveryLocationSelect.value === 'outside_dhaka') {
                deliveryCharge = 150;
            } else {
                deliveryCharge = 0;
            }
            ...
            deliveryChargeAmountSpan.textContent = deliveryCharge.toFixed(2);
            totalPayableAmountSpan.textContent = totalPayable.toFixed(2);
        }

        // Event listeners for delivery location and payment method
        if (deliveryLocationSelect) {
            deliveryLocationSelect.addEventListener('change', updateTotals);
        }
        ...
        if (checkoutForm) {
            checkoutForm.addEventListener('submit', (event) => {
                ...
                const orderDetails = {
                    products: newProducts.map(p => ({ id: p.id, name: p.name, quantity: p.quantity, price: p.price })),
                    customerName: document.getElementById('customerName').value,
                    customerPhone: document.getElementById('customerPhone').value,
                    deliveryAddress: document.getElementById('deliveryAddress').value,
                    deliveryLocation: document.getElementById('deliveryLocation').value,
                    paymentMethod: document.querySelector('input[name="paymentMethod"]:checked').value,
                    bkashTrxId: document.getElementById('bkashTrxId').value || null,
                    subtotal: subtotal,
                    deliveryCharge: deliveryCharge,
                    totalPayable: parseFloat(totalPayableAmountSpan.textContent)
                };
