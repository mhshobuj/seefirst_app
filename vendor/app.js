document.addEventListener('DOMContentLoaded', () => {
    ...
    async function handleAddProductPage() {
        ...
        const colorImagePairsContainer = document.getElementById('color-image-pairs');
        ...
        addColorImageBtn.addEventListener('click', () => {
            const pairDiv = document.createElement('div');
            pairDiv.className = 'color-image-pair';
            pairDiv.innerHTML = `
                <input type="text" placeholder="Color Name (e.g., Red)" class="color-name-input">
                <input type="file" class="color-image-input" accept="image/*">
            `;
            colorImagePairsContainer.appendChild(pairDiv);
        });
        ...
        addProductForm.addEventListener('submit', async (e) => {
            ...
            const colorImagePairs = document.querySelectorAll('.color-image-pair');
            colorImagePairs.forEach(pair => {
                const colorName = pair.querySelector('.color-name-input').value;
                const imageInput = pair.querySelector('.color-image-input');
                if (colorName && imageInput.files.length > 0) {
                    formData.append(`color_images_${colorName}`, imageInput.files[0]);
                }
            });
    ...
    async function handleProductDetailPage() {
        ...
        try {
            ...
            if (product.image) {
                const images = product.image.split(',');
                console.log('Default images:', images); // Debugging
                if (images.length > 0) {
                    mainImage.src = `http://localhost:3000/uploads/${images[0].trim()}`;
                }
                images.forEach(filename => {
                    if (filename) {
                        const thumb = document.createElement('img');
                        thumb.src = `http://localhost:3000/uploads/${filename.trim()}`;
            ...
            // Display color-wise images
            if (product.color_images) {
                try {
                    const colorImages = JSON.parse(product.color_images);
                    console.log('Color images:', colorImages); // Debugging
                    for (const color in colorImages) {
                        const colorDiv = document.createElement('div');
                        colorDiv.className = 'color-image-item';
                        colorDiv.innerHTML = `
                            <p><strong>${color}</strong></p>
                            <img src="http://localhost:3000/uploads/${colorImages[color]}" alt="${color} Image" width="100">
                        `;
